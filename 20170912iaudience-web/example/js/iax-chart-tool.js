var IAX_CHART_TOOL = {
  ...
  initFunnelGraph: function(json){
    ...
    // var graphData1 = $.extend({},json.graphData);
    // graphData1.brandName="ddf";
    // var graphData2 = $.extend({},json.graphData);
    // graphData2.brandName="123123";
    // var graphData3 = $.extend({},json.graphData);
    // graphData3.brandName="aa";
    // var graphData4 = $.extend({},json.graphData);
    // graphData4.brandName="bb";
    // var graphData5 = $.extend({},json.graphData);
    // graphData5.brandName="d";
  ...
  initTGI: function(genderId,ageGroupId,dataArr){
    ...
    for (var i = 0; i < dataArr.length; i++) {
      ...
      for (var j in gender_data.data) {
        ...
        if (dataArr.length==1) {
          ...
          gender_data.data[j].itemStyle.normal.color = _this.colorMap[i];
        }
      ...
      for (var j in ageGroup_data.data) {
        ...
        ageGroup_data.data[j].itemStyle.normal.color = _this.colorMap[i];
    ...
    var genderColorMap = dataArr.length>1 ? _this.colorMap : ["#4484CF","#ef4136","#c1c1c1"];
    _this.initBubbleBar(genderId,genderLegend,genderSeries,gender_data.xAxisData,genderColorMap);
    _this.initBubbleBar(ageGroupId,ageGroupLegend,ageGroupSeries,ageGroup_data.xAxisData,_this.colorMap);
  },
  initBubbleBar: function(id,legend,series,xAxisData,color){
    ...
    var _this = this;
    var myChart = echarts.init(document.getElementById(id));
    var splitNumber = 4,yaxisLabel=true,yaxisLabelFormatter="",legendshow = true,xAixsShow = true,gridleft=30,bottom=90,offsetX=30;
    var ymin=0,ymax=0;
  ...
  init_for_pdf: function(containerId){
    ...
    if (containerId == "for_export_product") {
      if ($("#"+containerId).find(".simulator-check-container").find(".selected").length>1) {
        //competitor brand
        $("#reports_pdf").find("#select_province_product_multi").parent().css("margin-top","280px");
        $("#reports_pdf").find("#select_province_product_multi").parent().prev().css("margin-top","280px");
        //my brand
        $("#reports_pdf").find("#select_province_multi").parent().css("margin-top","280px");
        $("#reports_pdf").find("#select_province_multi").parent().prev().css("margin-top","280px");
      };
    };
    if (containerId == "for_export_brand") {
      if ($("#"+containerId).find(".simulator-check-container").find(".selected").length>1) {
        //competitor product
        $("#reports_pdf").find("#select_province_multi").parent().css("margin-top","480px");
        $("#reports_pdf").find("#select_province_multi").parent().prev().css("margin-top","480px");
