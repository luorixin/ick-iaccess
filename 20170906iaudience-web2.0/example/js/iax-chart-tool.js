var IAX_CHART_TOOL = {
  ...
  initTop5Trend:function(id,data){
    ...
    var _this = this;
    var legends=[],date=[],series=[],seriesJson={},legendJson={},title,dateJson={},convertResult=[];
    title = "";
    for (var i in data) {
      if (data[i]) {
  ...
  initTotalAudienceMulti: function(mainId,nameId,scaleId,shareId,data){
    ...
    for(var i in data){
      if (data[i]) {
        ...
        nameList.push('<li style="line-height:25px;"><span style="width:8px;height:8px;border-radius:8px!important;background:'+this.colorMap[index]+';display:inline-block;margin-right:5px;"></span>'+data[i].productName+'</li>');
  ...
  initAgeGroupMulti: function(id,data){
    if (!data) return;
    if (!document.getElementById(id)) return;
    var ageGroupArr=[],ageGroupSeries=[],ageGroupJson={},ageGroupLegend=[];
    var _this = this;
    //处理age group数据
    var group_data={},xAxisData_age_group=[];
    for (var i in data) {
    ...
    _this.initBarFunnel(id,ageGroupLegend,ageGroupSeries,xAxisData_age_group,this.colorMap);
  },
  ...
  initProductShareMulti: function(id,data){
    ...
    if (!document.getElementById(id)) return;
    var _this = this;
    var legendData = [], seriesData = [],graphColorMap=[];maxLen=60,length=0,itemGap=10,width=parseInt($("#"+id).css("width"));
    var colorMapArr = [{color:this.colorMap[0],_colorMap:['#FF4343','#F2675E','#F7A09A','#FBCFCD']},{color:this.colorMap[1],_colorMap:['#FFBD00','#FFCE40','#FFDE7F','#FFEEBF']},{color:this.colorMap[2],_colorMap:['#4484CF','#73A3DB','#A1C1E7','#D0E0F3']},{color:this.colorMap[3],_colorMap:['#946EDB','#AF93E4','#C9B6ED','#E4DBF6']},{color:this.colorMap[4],_colorMap:['#8D7B7B','#AA9C9C','#C6BDBD','#E2DEDE;']}]
    $("#"+id).empty();
    var index = 0;
    for(var k in data){
      if (data[k]) {
        var graphid = "graph_brand_"+id+k;
        legendData = [], seriesData = [],graphColorMap=colorMapArr[index]._colorMap;
        var _total = 0;
        ...
        for (var i = 0; i < legendData.length; i++) {
          //初始化副标题
          liHtml += '<li title="'+legendData[i].name+'"><span style="width:8px;height:8px;border-radius:8px!important;background:'+colorMapArr[index]._colorMap[i]+';display:inline-block;margin-right:5px;"></span>'+legendData[i].name+'</li>';
        } 
    ...
    for(var k in data){
      if (data[k]) {
        ...
        var conhtml = '<div class="interest-multi-container">'+
                    ...
                    '</div>';  
        $("#"+id).append(conhtml);     
        this.createSmallSpiderByHighChart(graphid,datas,categories,liCon,this.colorMap[index]);
        index++;
      };
    }
     //调整距离
    ...
    if (id=="reports_pdf") {
      ...
      if($("#"+id).find(".interest-multi-graph").length>0){
        $("#"+id).find(".interest-multi-graph").each(function(){
          ...
          canvg(document.getElementById("result_canvas_"+graphid),svgHtml);
        })
        
      }
      //背景色问题
      var $male = $("#"+id).find(".gender-male");
      var margin = "40px auto 10px auto;";
      //特殊情况。。multi的时候
      if ($male.parent().hasClass("male_multi")) {
        margin = "40px -100% 10px -60px";
      };
      $male.find(".gender-map-ico").after('<img src="../images/icon_male_chart.png" style="margin:'+margin+'">');
  ...
  init_for_pdf: function(containerId){
    ...
    if (containerId == "for_export_product") {
      if ($("#"+containerId).find(".simulator-check-container").find(".selected").length>1) {
        $("#reports_pdf").find("#select_province_product_multi").parent().css("margin-top","400px");

      };
    };
    if (containerId == "for_export_brand") {
      if ($("#"+containerId).find(".simulator-check-container").find(".selected").length>1) {
        $("#reports_pdf").find("#select_province_multi").parent().css("margin-top","600px");
