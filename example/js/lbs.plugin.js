...
(function($){
    ...
    var methods = {
        init: function(options){
            ...
            this.each(function(){
                ...
                var anaDB = _findByKey(opts.lbsData, 'keyword','all');
                self.data('base_option',{
                    ...
                    all_analysis_db:opts.lbsData,// 存储当前请求过来包含多个关键词的数据
                    analysis_db:anaDB, // 存储当前请求过来多个城市的数据
                    ...
                    circleColor:['#4484CF','#77DB45','#FFBD00','#C77A75','#8F7FAE','#A29186'],
                    LEVEL_1_ZOOM : 11,
                    LEVEL_2_ZOOM : 14,
                    LEVEL_3_ZOOM : 17
                });
                self.data('all_analysis_db',opts.lbsData);
                self.data('analysis_db',anaDB);
                self.data('filter',opts.params.filter);
    ...
    function _render(elem,opts){
        ...
        var translate = $.fn.lbs.langConfig[opts.lang],
            ...
            brandHtml =  '<div class="lbs-info-detail">'+
                ...
                ' </div>'+
                '</div>',
            keywordsLi = opts.params.keywords.map(function(v,i){return "<li value='"+v+"'>"+v+"</li>"}).join(""),
            keywordsHtml =  '<div class="lbs-info-detail">'+
                '  <label class="info-detail-title">'+translate["keywords"]+'</label>'+
                '  <div class="info-detail-con">'+
                '    <ul class="tag keywords-btn">'+
                '       <li class="keyword-active" value="all">'+translate["keywords_all"]+'</li>'+
                keywordsLi +
                '    </ul>'+
                '  </div>'+
                '</div>',
    ...
    function _bindEvent($elem,opts){
        ...
        $elem.find('.keywords-btn>li').on('click',function(){
            if (!base_option.all_analysis_db) return;
            //用具体的关键词 取相应关键词下的数据
            var _tmp_analysis_db = _findByKey(base_option.all_analysis_db, 'keyword',$(this).attr("value"));;
            analysis_db = _tmp_analysis_db;//更新当前关键词其下的数据
            $(this).siblings().removeClass('keyword-active');
            $(this).addClass('keyword-active');

            var _val = $(this).attr("value") == 'all' ? '' : '：'+$(this).attr("value");
            // $('.current-keyword').text(_val);
            //如果不是all 的话 其他关键词 让左侧checkbox变灰 不可选择
            if(_isAllKeywordSelected($elem)){
                $elem.find(".lbs-container").removeClass('not-allowed');
            }else{
                $elem.find(".lbs-container").addClass('not-allowed');
            }

            setTimeout(function(){//防止快速切换时渲染错乱
                var _cur_city_code = $elem.find('.lbs-container').attr('cur_city_code');
                // $('#current-city-name').html($('#selected-city .lbs-map-active').text());
                if(!analysis_db || analysis_db.city.length == 0){
                    _noDataChange($elem, {level:'keyword',city:city_map[_cur_city_code]});
                    return;   
                }else{
                    var _select_city_data = _findByKey(analysis_db.city, 'cityCode',_cur_city_code);
                    if(_select_city_data){//该关键词下有数据 并且当前选中城市有数据
                        _filterChange($elem, _select_city_data.city, _select_city_data.cityCode); 
                        _cityChangeInit($elem, _select_city_data,_select_city_data.cityCode);
                    }else{
                        //让所有汇总数据更新 防止上次结果导致错误
                        _magicNumber($elem.find("[name='impressions_sum']").find("p").last(), analysis_db.total.pv);
                        _magicNumber($elem.find("[name='audience_sum']").find("p").last(), analysis_db.total.uv);
                        _noDataChange($elem, {level:'city',city:city_map[_cur_city_code]});
                    }
                }
    ...
    function _getData(elem,opts){
        var _target_data;
        if (opts.lbsData) {
            var analysis_db = _findByKey(opts.lbsData, 'keyword','all');
            if (analysis_db.city && analysis_db.city.length>0) {
                for(var i = 0; i<opts.params.cities.length; i++){
                    var _tmp_city = opts.params.cities[i];
                    _target_data = _findByKey(analysis_db.city,'city',city_map[_tmp_city.id]);
                    if(_target_data){
                        cityId = _tmp_city.id;
                        if (elem) {
    ...
    function _filterChange($elem,name,id){
        ...
        var base_option = $elem.data('base_option');
        var _tmp_filter = _findByKey(filter,'city', name);
        if (_tmp_filter) {
            ...
            base_option.current_city_filter = _findByKey(filter,'city', name);
        }
    }

    function _noDataChange($elem, option){
        var base_option = $elem.data('base_option');
        base_option.mapObj.setCity(option.city); 
        ...
        // $('#district-filter,#street-filter').empty();
        $elem.find(".district-filter").empty();
        $elem.find(".district-filter").trigger("liszt:updated");
        $elem.find(".street-filter").empty();
        $elem.find(".street-filter").trigger("liszt:updated");

        $elem.find('.lbs-item-list').html('');
        AMap.event.removeListener(base_option.zoomListener); 
        $elem.find("[name='impressions_sum']").find("p").last().text(0);
        $elem.find("[name='audience_sum']").find("p").last().text(0);
        if(option.level == 'city'){
            
        }else{
            $('#pv-total-city,#uv-total-city').text(0);
        }
    ...
    function _cityChangeInit($elem,data){
        ...
        //如果城市是香港和澳门则只能查看区级数据
        city_code = $elem.find('.lbs-container').attr('cur_city_code');

        _initMap($elem,base_option.current_district[0]);
        _addZoomListener($elem);
        ...
        base_option.disable_street = $.inArray(city_code,no_street_data) >= 0;
        if(base_option.disable_street){
            $elem.find('.area-level-box li:gt(0)').addClass('disabled');
        }else{
            $elem.find('.area-level-box li:gt(0)').removeClass('disabled');
        }

        setTimeout(function(){
            base_option.disable_street && _addZoomListener($elem,false);
        },500);
    ...
    function _addZoomListener($elem,isAdd){
        var base_option = $elem.data('base_option');
        disable_street = base_option.disable_street
        if(disable_street) return;
        if (isAdd) {
    ...
    function _isAllKeywordSelected($elem){//判断当前关键词是否是选中All 
        return $elem.find('.lbs-info-detail').find('li.keyword-active').index() == 0;
    }
    ...
    function _createMarkerDom($elem,item,level){
            ...
            if (_tmp_target && _tmp_target.checked == 1) {
                ...
                for (var i = 0; i < base_option.analysis_result.street.length; i++) {
                    ...
                    if (_findByKey(_tmp_street_array,'name', _tmp_street_item.name)) {
                        _tmp_pv += parseFloat(_tmp_street_item.pv);
                        _tmp_uv += parseFloat(_tmp_street_item.uv);
                    }
                }
                for(var j=0; j<_tmp_poi_array.length; j++){
                    ...
                    if(_tmp_cur_poi){
                        _tmp_pv += parseFloat(_tmp_cur_poi.pv);
                        _tmp_uv += parseFloat(_tmp_cur_poi.uv);
                    }
                }
                item.pv  -= _tmp_pv;
                item.uv  -= _tmp_uv;
        ...
        if(level == 2){
            marker_offset = new AMap.Pixel(-20, -100);
        }else{
            marker_offset = new AMap.Pixel(-20, -34);
        }
        var _tmp_content = _isAllKeywordSelected($elem) ? _createInfoBox($elem, item, excluded) : _createInfoBox($elem, original_item);
        ...
        var active_keyword = $elem.find('.lbs-info-detail').find('li.keyword-active')
        var active_keyword_color = active_keyword[0] ? base_option.circleColor[active_keyword.index()] : base_option.fillColor.normal;
        var _cur_keyword_color = _isAllKeywordSelected($elem) && excluded ? base_option.fillColor.gray : active_keyword_color;
        ...
        var circle = new AMap.Circle({
            ...
            strokeWeight: 3, //线粗细度
            zIndex: 1000,
            fillColor: _cur_keyword_color , //填充颜色
    ...
    function _createInfoBox($elem,item, excluded){
        ...
        var check_class = 'fa fa-check-square';
        if(_isAllKeywordSelected($elem)){                    
            check_class = $elem.find('.lbs-area-box').find('label[location="'+_location+'"]').find("i").attr("class");
        }
        content = '<div location="'+_location+'" data-id="" class="lbs-infowindow-box ' + exclude_class + '" style="position:relative;" >' + '<img src="../images/marker.png" alt="" />' + '<div class="lbs-infowindow" >' + '<p class="name" ><label location="'+_location+'" title="'+item.name+'"><i class="'+check_class+'"></i><em>' + item.name + '</em></label></p>' + '<p>Est. Impressions：<span class="pv-num">' + _commaft(item.pv) + '</span></p>' + '<p>No. of Audience：<span class="uv-num">' + _commaft(item.uv) + '</span></p>' + '<img class="arrow" src="../images/icon_tip.png" alt="" />' + '</div></div>';
        return content;
    }
    ...
    function _maplistItemClick($elem){
        ...
        $elem.on('click', '.lbs-area-box label', function(ev) {
            ...
            if (target_dom == 'EM'){//如果点击的是文本进入其下级
                !disable_street && _markerClick($elem,_name,_level);
                return;
            };

            if(!_isAllKeywordSelected($elem)) return;
            ...
            //可能没生成导致找不到，如果找不到就从marker中获取
            if (tmp_infowindow.length==0) {
                var _point = base_option.markers.filter(function(item){
                    var _tmp_data = item.getExtData();
                    return _tmp_data.name == _name && _tmp_data.type == 'point';
                });
               tmp_infowindow = $(_point[0].getContentDom()).find('.lbs-infowindow-box');
            };
    ...
    function _computedCityFilter($elem, filter_data){
        var base_option = $elem.data('base_option');
        var filter = $elem.data('filter');
        var analysis_db = $elem.data('analysis_db');
        var total_pv = 0;
        ...
        var city_filter = function(cur_filter) {
            ...
            for (var i = 0; i < init_data.district.length; i++) {
                ...
                if (_findByKey(tmp_district,'name', _tmp.name)) {
                    total_pv += parseFloat(_tmp.pv);
                    total_uv += parseFloat(_tmp.uv);
                }
            }
            ...
            for(var i=0; i < tmp_street.length; i++ ){
                ...
                total_pv += parseFloat(_tmp_target_street.pv);
                total_uv += parseFloat(_tmp_target_street.uv);
            }
            for (var i = 0; i < cur_filter.poi.length; i++) {
                ...
                total_pv += parseFloat(_tmp_data_poi[0].pv);
                total_uv += parseFloat(_tmp_data_poi[0].uv);
            }

        };

        if(Object.prototype.toString.call(filter_data) == '[object Object]'){
           filter_data = [filter_data]; 
        }

        for (var i = 0; i < filter_data.length; i++) {
            city_filter(filter_data[i]);
        }

        return {pv:total_pv,uv:total_uv};
    ...
    function _totalComputed($elem) {
        ...
        var _tmp_all_filter_total = _computedCityFilter($elem, filter);

        //当前选中的城市
        var _active_city =  $elem.find('.lbs-container').attr('cur_city_code');
        var active_city_data = _findByKey(analysis_db.city, 'cityCode',_active_city)

        if(!_isAllKeywordSelected($elem)){
            _magicNumber($elem.find("[name='impressions_sum']").find("p").last(), total.pv);
            _magicNumber($elem.find("[name='audience_sum']").find("p").last(), total.uv);
            // $('#pv-total-city').magicNumber(active_city_data.total.pv);
            // $('#uv-total-city').magicNumber(active_city_data.total.uv);
        }else{
            var _tmp_filter_total = _computedCityFilter($elem, _findByKey(filter, 'code',_active_city));

            console.log('_tmp_filter_total',_tmp_filter_total,active_city_data)

            _magicNumber($elem.find("[name='impressions_sum']").find("p").last() , total.pv - _tmp_all_filter_total.pv);
            _magicNumber($elem.find("[name='audience_sum']").find("p").last(), total.uv - _tmp_all_filter_total.uv);
            // $('#pv-total-city').magicNumber(active_city_data.total.pv - _tmp_filter_total.pv);
            // $('#uv-total-city').magicNumber(active_city_data.total.uv - _tmp_filter_total.uv);
        }
    }

    function _magicNumber($elem, value, duration) {// value 为字符串的数字 或数字
        var n = 0;
        ...
        var step = parseInt(50 * value / duration);
    ...
    function _createList($elem,list, index) {
        ...
        var createItem = function(item, ii, level) {
            ...
            var check_class;
            if(!_isAllKeywordSelected($elem)){
                check_class = 'fa-check-square';
            }else{
                check_class = item.checked == 0 ? 'fa-square-o' : (item.checked == 1 ? 'fa-minus-square' : 'fa-check-square');
            }
    ...
    function _isChanged(current_params,params){
        ...
        var currentCity = current_params.cities.map(function(thi){
            return thi.id;
        }).join(",");
        var newCity = params.cities.map(function(thi){
            return thi.id;
        }).join(",");
    ...
    $.fn.lbs.defaults = {
        ...
        cityJson:'',
        isReload:false,
        params:{name:'',keywords:[],distance:"1",cities:[],filter:[]},
        ...
        onSave: function(opts){
            ...
            var pdata = {
                ...
                cities: opts.params.cities,
                filter: self.data("filter"),
                distance: opts.params.distance,
                keywordsDefault: opts.params.keywordsDefault,
        ...
        onEdit:function(opts){
            ...
            self.find(".modal").modal("show");
            setTimeout(function(){
                    var isVisible = self.find(".modal").hasClass('in');
                    if(isVisible){
    ...
    $.fn.lbs.langConfig = {
        ...
        'en':{
            ...
            'save':'Save Locations',
            'impressions':'Estimated Impressions',
            'audience':'Number of Audience',
            'impressions_small':'Est. Impressions',
            'audience_small':'No. of Audience',
            'footFall':'Hyper Location',
            'heatMap':'Heat Map',
            'save':'Save',
            'cancel':'Cancel',
            'addkeyword':'Add a keyword',
