(function($){
	...
	function _render(elem,opts){
		...
		$selectArea.find("ul").each(function(index, element) {
          ...
          var left = 5+_rate*15;
          if(ulContent){
            $(element).prev().after('<i style="position:absolute;cursor:pointer;color:#999;top:9px;left:'+left+'px;" class="fa '+showIcon+' expand-button"></i>') ;
          }
        });
		...
		if (opts.isRevert) {
			elem.find("li").each(function(){
				_check($(this).find("i").first(),true);
			})
		};
		var results = elem.data('results');
		var resultsLevel = "[rate=1]";
		if (!opts.isRelation && opts.isReturnAll) {
    		resultsLevel = ""
    	};
		if (results.length>0) {
			for (var i = 0; i < results.length; i++) {
				var id = results[i];
				elem.find("ul"+resultsLevel).find("li[_id='"+id+"']").find("a:first").trigger("click");
			};
		};
	}
		...
		if (data && data.length>0) {
			...
			for (var i = 0; i < data.length; i++) {
	            ...
	            if(data[i]["children"] != null && data[i]["children"].length>0){
	               liDoms += '<ul class="childs" style="display:'+childsDisplay+'" rate="'+rate+'">';
	               rate++;//递归时候才展示间距
	...
	function _bindEvent($elem,opts){
        ...
        $elem.on("click",".dropdown-tree .dropdown-menu a",function(){
        	...
        	//结果展示
        	var results = $elem.data('results');
        	var resultsClass = ".fa-check-square";
        	var resultsLevel = "[rate=1]";
        	if (opts.isRevert) {
        		resultsClass = ".fa-square-o";
        	};
        	if (opts.isReturnAll) {
        		resultsLevel = ""
        	};
        	results = $elem.find("ul"+resultsLevel).find(resultsClass).map(function(){
