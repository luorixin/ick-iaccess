var IAX_CHART_TOOL = {
  ...
  loadingGif:function(id){
    ...
    var html ='<div style="width:500px;left:50%;margin-top:20px;margin-left:-200px;position:relative">'+
              ...
              '    <p style="display:table;text-align:left;height:60px;"><label style="display:table-cell;vertical-align: middle;font-size:20px;line-height:30px;">Defining the market<dot>...</dot></label></p><hr/>'+
              '    <p style="color:#999;text-align:left;width:300px;font-size:12px;">Audience plan is running and that can take a few moments. Hang in there, the analysis will be worth the wait.</p>'+
              '  </div>'+
              '</div>';
    $("#"+id).append('<div class="analysis-loading">'+html+'</div>');
    var changeText = ["Defining the market<dot>...</dot>"," Looking for your brand audience<dot>...</dot> ","Analysing your brand keywords and products<dot>...</dot>","Searching for your brand related LBS audience<dot>...</dot>","Looking for your brand potential audience<dot>...</dot> ","Looking for competitor audience<dot>...</dot>"," Analysing your competitor keywords and products<dot>...</dot>","Comparing your brand with competitors<dot>...</dot>","Comparing your brand products with competitor products<dot>...</dot>","Loading the audience plan result<dot>...</dot>"];
    function setProcess(terminal){  
      ...
      processbar.parent().parent().next().find("p:first").find("label").html(text);
    ...
    for (var i in data.mixTrend.result) {
      ...
      if (yAxisIndex==0) {
        yAxisJson.axisLabel.formatter = function (value, index) {
            return _this.formatNum(_this.getKWMformat(value),2);
        }
      };
      if (yAxisIndex==1) {
        // yAxisJson.max=100;
  ...
  initProductAssociaton: function(id,data,isChage,ytype){
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      timeCircle.marginTop = -timeCircle.r -5;
    ...
    if (isChage) {
      for (var i = 0; i < times.length; i++) {
        ...
        var innerhtml = '<span style="'+direct+':-30px;position:absolute;width:200px;transform: translateX(-50%);text-align:center;">'+times[i].name+'</span>'
  ...
  initRegions: function(id,data){
    ...
    var option = {
      ...
      //省份城市选择tab
      var selectTab = '<ul class="region-select-tab" style="visibility:hidden">'+
                        '<li class="selected" data-value="0">Provicne</li>'+
                        '<li class="" data-value="1">City</li></ul>'+
                      '</ul><hr style="width:100%;visibility:hidden;display: inline-block;margin: 0px 0 10px 0;"/>';
  ...
  initRegionsCity: function(id,data,city){
    ...
    geoJson =HKjson;
    // $.get('./js/echarts-map/'+city.smallName+'.json',
    //   function(geoJson) {
