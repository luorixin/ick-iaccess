var IAX_CHART_TOOL = {
  ...
  initBubbleCross: function(colorMap,id,data,xy,xArea,yArea){
    ...
    //动态获取x,y轴的最小最大值
    var xAreaMin=0,xAreaMax=0,yAreaMin=0,yAreaMax=0;
    for(var i in data){
      if(xAreaMin>data[i][0][0]){
        xAreaMin = data[i][0][0];  
      }
      if(xAreaMax<data[i][0][0]){
        xAreaMax = data[i][0][0];  
      }
      if(yAreaMin>data[i][0][1]){
        yAreaMin = data[i][0][1];  
      }
    ...
    xArea = [xAreaMin-10,xAreaMax+10];
    yArea = [yAreaMin-10,yAreaMax+10];
    echarts.dispose(document.getElementById(id));
    var myChart = echarts.init(document.getElementById(id));
    var _this = this;
    var legend=[];
    ...
    var picSeriesArr = [],sizeBubble=[40,45,50,55,60,65,70,75,80],labeLimit=[5,5,6,7,9,10,11,12,13];
    ...
    for(var i in data){
      ...
      legend.push(i);
      seriesIndexCountArr[i] = seriesIndexCount;
      seriesIndexCount++;
    }
    ...
    var option = {
        ...
        tooltip: {
            ...
            formatter: function (params) {
              // params = params[0]
              if (params.seriesName!="test") {
                return "<p><b>"+params.seriesName+"</b></p><p>"+xy[1].split(' (%)')[0]+": "+params.value[3]+"%</p><p>"+xy[0].split(' (%)')[0]+": "+_this.formatNum(params.value[2],0)+"%</p>";
              }
            },
            // axisPointer:{
            //     show: true,
